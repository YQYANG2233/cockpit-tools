...
pub type LocalAccessEventPublisher = Arc<dyn Fn(&str, Value) + Send + Sync + 'static>;
static LOCAL_ACCESS_EVENT_PUBLISHER: OnceLock<Mutex<Option<LocalAccessEventPublisher>>> =
    OnceLock::new();

pub fn set_local_access_event_publisher(publisher: Option<LocalAccessEventPublisher>) {
    if let Ok(mut current) = LOCAL_ACCESS_EVENT_PUBLISHER
...
fn publish_local_access_event(event: &str, payload: Value) {
    let publisher = LOCAL_ACCESS_EVENT_PUBLISHER
        .get_or_init(|| Mutex::new(None))
        .lock()
        .ok()
...
fn sanitize_collection_with_accounts(
    ...
    let valid_bound_oauth_account_ids: HashSet<String> = accounts
        .iter()
        .filter(|account| !account.is_api_key_auth() && account_has_refresh_token(account))
...
fn emit_chat_test_stream_event(session_id: &str, payload: Value) {
...
async fn run_local_access_chat_stream_dialog(
    ...
    while let Some(chunk_result) = stream.next().await {
        ...
        while let Some(index) = buffer.find("\n\n") {
            ...
            if handle_local_access_chat_stream_frame(session_id, &frame) {
    ...
    if !buffer.trim().is_empty() && handle_local_access_chat_stream_frame(session_id, &buffer) {
        emit_chat_test_stream_event(
            session_id,
            json!({
                "type": "done",
...
fn handle_local_access_chat_stream_frame(session_id: &str, frame: &str) -> bool {
    let data = frame
        .lines()
