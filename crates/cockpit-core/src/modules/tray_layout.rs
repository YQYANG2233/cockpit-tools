        ...
        Err(error) => {
            match crate::modules::atomic_write::quarantine_file(&path, "invalid-json") {
                Ok(Some(backup_path)) => crate::modules::logger::log_warn(&format!(
                    "Tray layout config parse failed; quarantined and using default: path={}, backup={}, error={}",
                    path.display(),
                    backup_path.display(),
                    error
                )),
                Ok(None) => crate::modules::logger::log_warn(&format!(
                    "Tray layout config parse failed; file no longer exists, using default: path={}, error={}",
                    path.display(),
                    error
                )),
                Err(backup_error) => crate::modules::logger::log_warn(&format!(
                    "Tray layout config parse failed; quarantine failed, using default: path={}, parse_error={}, backup_error={}",
                    path.display(),
                    error,
                    backup_error
                )),
...
pub fn save_tray_layout(
    ...
    let content = serde_json::to_string_pretty(&normalized)
        .map_err(|err| format!("serialize tray layout config failed: {err}"))?;
    crate::modules::atomic_write::write_string_atomic(&path, &content)
        .map_err(|err| format!("save tray layout config failed: {err}"))?;
    Ok(normalized)
}
