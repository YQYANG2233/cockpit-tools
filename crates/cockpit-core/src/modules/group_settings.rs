...
pub fn replace_group_settings(
    group_mappings: HashMap<String, String>,
    group_names: HashMap<String, String>,
    group_order: Vec<String>,
) -> Result<(), String> {
    let mut settings = load_group_settings();
    settings.group_mappings = group_mappings;
    settings.group_names = group_names;
    settings.group_order = group_order;
    settings.updated_at = chrono::Utc::now().timestamp_millis();
    settings.updated_by = ConfigSource::Desktop;
    update_group_settings(settings)
}

pub fn mutate_group_settings<F>(mutator: F) -> Result<(), String>
where
...
pub fn get_display_groups() -> Vec<DisplayGroup> {
    let settings = load_group_settings();
    settings
        .get_ordered_groups(Some(3))
        .into_iter()
        .map(|group_id| {
            let models = settings.get_models_in_group(&group_id);
            DisplayGroup {
                id: group_id.clone(),
                name: settings.get_group_name(&group_id),
                models,
            }
        })
        .collect()
