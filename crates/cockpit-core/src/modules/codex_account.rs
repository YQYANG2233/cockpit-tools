...
> = std::sync::LazyLock::new(|| Mutex::new(HashMap::new()));
type CodexBatchImportEventPublisher = dyn Fn(&str, serde_json::Value) + Send + Sync + 'static;
static CODEX_BATCH_IMPORT_EVENT_PUBLISHER: LazyLock<
    Mutex<Option<Arc<CodexBatchImportEventPublisher>>>,
> = LazyLock::new(|| Mutex::new(None));
static CODEX_BATCH_IMPORT_SESSIONS: LazyLock<Mutex<HashMap<String, CodexBatchImportSession>>> =
...
struct CodexBatchImportSession {
    ...
    status: String,
    items: Vec<CodexBatchImportWorkItem>,
    cancel_requested: bool,
}

#[derive(Clone)]
struct CodexBatchImportWorkItem {
    preview: CodexBatchImportItem,
    candidate: Option<CodexBatchImportCandidate>,
    auth_file_plan_type: Option<String>,
}

#[derive(Clone)]
enum CodexBatchImportCandidate {
...
pub fn set_codex_batch_import_event_publisher(
    publisher: Option<Arc<CodexBatchImportEventPublisher>>,
) {
    if let Ok(mut slot) = CODEX_BATCH_IMPORT_EVENT_PUBLISHER.lock() {
        *slot = publisher;
    }
}

fn publish_codex_batch_import_event<T>(event: &str, payload: &T)
where
    T: serde::Serialize,
{
    let Some(publisher) = CODEX_BATCH_IMPORT_EVENT_PUBLISHER
        .lock()
        .ok()
...
fn build_batch_import_preview(
    session_id: &str,
    status: &str,
    items: &[CodexBatchImportWorkItem],
...
fn count_batch_import_progress_items(
    items: &[CodexBatchImportWorkItem],
    ...
    let failed = items
        ...
        .count();
    let quota_failed = items
        .iter()
        .filter(|item| item.preview.status == "quota_failed")
        .count();
    let existing = items
        .iter()
        .filter(|item| item.preview.status == "existing")
        .count();
    (success, failed, quota_failed, existing)
}

fn publish_batch_import_progress(
    session_id: &str,
    phase: &str,
    current: usize,
    total: usize,
    ...
    let (success, failed, quota_failed, existing) = count_batch_import_progress_items(items);
    publish_codex_batch_import_event(
...
fn publish_file_import_progress(current: usize, total: usize, email: &str) {
    publish_codex_batch_import_event(
...
fn invalid_batch_import_item(
    item_id: String,
    source: String,
    label: String,
    error: String,
) -> CodexBatchImportWorkItem {
    CodexBatchImportWorkItem {
        preview: CodexBatchImportItem {
            item_id,
            source,
            ...
            provider: None,
            quota_status: "invalid".to_string(),
            quota_error: None,
...
fn batch_import_source_label(path: &Path, fallback: &str) -> String {
    path.file_name()
        .and_then(|name| name.to_str())
        .and_then(|name| normalize_optional_ref(Some(name)))
        .unwrap_or_else(|| fallback.to_string())
}

fn batch_import_file_stem_label(path: &Path) -> String {
    path.file_stem()
        .and_then(|name| name.to_str())
        .and_then(|name| normalize_optional_ref(Some(name)))
...
fn candidate_item_id(session_id: &str, ordinal: usize) -> String {
    format!("{}-{}", session_id, ordinal)
}

fn batch_candidate_from_json_value(value: serde_json::Value) -> CodexBatchImportCandidate {
    if let Some((tokens, account_id_hint)) = extract_codex_tokens_from_value(&value) {
        CodexBatchImportCandidate::LegacyTokens {
            tokens,
            account_id_hint,
        }
    } else {
        CodexBatchImportCandidate::JsonValue(value)
    }
}

fn label_from_json_value(value: &serde_json::Value, fallback: &str) -> String {
    first_json_string(
        value,
        &[
            &["email"],
    ...
    .or_else(|| extract_account_note_from_value(value))
    .unwrap_or_else(|| fallback.to_string())
}

fn preview_existing_from_identity(
    email: &str,
    account_id: Option<&str>,
    organization_id: Option<&str>,
) -> Option<String> {
    let index = load_account_index();
    find_existing_account_id(&index, email, account_id, organization_id)
}

fn preview_oauth_candidate(
    item_id: String,
    source: String,
    label: String,
    tokens: &CodexTokens,
    account_id_hint: Option<&str>,
) -> CodexBatchImportItem {
    match extract_user_info(&tokens.id_token) {
        Ok((
            email,
            _user_id,
            plan_type,
            _subscription_active_until,
            token_account_id,
            token_org_id,
        )) => {
            let account_id = normalize_optional_value(
                extract_chatgpt_account_id_from_access_token(&tokens.access_token)
                    .or(token_account_id)
                    .or_else(|| account_id_hint.map(|value| value.to_string())),
            );
            let organization_id = normalize_optional_value(
                extract_chatgpt_organization_id_from_access_token(&tokens.access_token)
                    .or(token_org_id),
            );
            let existing_id = preview_existing_from_identity(
                &email,
                account_id.as_deref(),
                organization_id.as_deref(),
            );
            let existing = existing_id.is_some();
            CodexBatchImportItem {
                item_id,
                source,
                label: if email.trim().is_empty() {
                    label
                } else {
                    email.clone()
                },
                account_id: existing_id.or(account_id),
                email: Some(email),
                account_type: "oauth".to_string(),
                provider: plan_type,
                quota_status: "not_checked".to_string(),
                quota_error: None,
                status: if existing { "existing" } else { "ready" }.to_string(),
                error: None,
        ...
        Err(error) => CodexBatchImportItem {
            item_id,
            source,
            label,
            account_id: None,
...
fn preview_access_token_candidate(
    item_id: String,
    source: String,
    label: String,
    access_token: &str,
    ...
    let (email, _user_id, plan_type, _subscription_active_until, account_id, organization_id) =
        extract_access_token_identity(access_token);
    let email = email
        .or_else(|| account_id.as_ref().map(|value| format!("codex-{}", value)))
        .unwrap_or_else(|| {
            account_note
                ...
                .unwrap_or_else(|| {
                    format!("codex-access-{}", access_token_fingerprint(access_token))
                })
        });
    let existing_id =
        preview_existing_from_identity(&email, account_id.as_deref(), organization_id.as_deref());
    let existing = existing_id.is_some();
    CodexBatchImportItem {
        item_id,
        source,
        ...
        account_id: existing_id.or(account_id),
        email: Some(email),
        account_type: "access_token".to_string(),
        provider: plan_type,
        quota_status: "not_checked".to_string(),
        quota_error: None,
        status: if existing { "existing" } else { "ready" }.to_string(),
...
fn preview_refresh_token_candidate(
    item_id: String,
    source: String,
    label: String,
    refresh_token: &str,
    ...
    let label = account_note
        .map(|value| value.to_string())
        .unwrap_or_else(|| {
            if label.trim().is_empty() {
                let digest = format!("{:x}", md5::compute(refresh_token.as_bytes()));
                format!("refresh-token-{}", &digest[..8])
            } else {
                label
            }
        });
    CodexBatchImportItem {
        ...
        account_id: None,
        email: None,
        account_type: "refresh_token".to_string(),
        provider: None,
        quota_status: "not_checked".to_string(),
        quota_error: None,
...
fn preview_api_key_value(
    item_id: String,
    source: String,
    label: String,
    value: &serde_json::Value,
) -> Option<CodexBatchImportItem> {
    if !is_auth_mode_apikey(
        value
            .get("auth_mode")
            .and_then(|value| value.as_str())
    ...
    let api_key = value
        ...
        .and_then(normalize_api_key)?;
    let account_id = build_api_key_account_id(&api_key);
    let existing = load_account(&account_id).is_some();
    let provider = value
        .get("api_provider_name")
        .and_then(|value| value.as_str())
        .and_then(|value| normalize_optional_ref(Some(value)));
    Some(CodexBatchImportItem {
        item_id,
        source,
        label: value
            .get("email")
...
fn preview_import_candidate(
    ...
    match candidate {
        CodexJsonImportCandidate::FullToken {
            ...
            account_id_hint,
            ..
        } => preview_oauth_candidate(item_id, source, label, tokens, account_id_hint.as_deref()),
        CodexJsonImportCandidate::AccessToken {
            access_token,
            account_note,
        } => preview_access_token_candidate(
            item_id,
            source,
            label,
            access_token,
            account_note.as_deref(),
        ),
        CodexJsonImportCandidate::RefreshToken {
            refresh_token,
            account_note,
        } => preview_refresh_token_candidate(
            item_id,
            source,
            label,
            refresh_token,
...
fn preview_account_struct(
    ...
    CodexBatchImportItem {
        item_id,
        source,
        label: if account.email.trim().is_empty() {
            label
        } else {
            account.email.clone()
        },
        ...
        account_type: if account.is_api_key_auth() {
            "api_key".to_string()
        } else {
            "oauth".to_string()
        },
...
fn build_batch_import_work_item(
    item_id: String,
    source: String,
    label: String,
    candidate: CodexBatchImportCandidate,
    auth_file_plan_type: Option<String>,
) -> CodexBatchImportWorkItem {
    let preview = match &candidate {
        ...
        CodexBatchImportCandidate::JsonValue(value) => {
            preview_api_key_value(item_id.clone(), source.clone(), label.clone(), value)
                .or_else(|| {
                    extract_codex_import_candidate_from_value(value).map(|candidate| {
                ...
                .or_else(|| {
                    serde_json::from_value::<CodexAccount>(value.clone())
                        .ok()
                        .map(|account| {
                            preview_account_struct(
                                item_id.clone(),
                                source.clone(),
                                label.clone(),
                                &account,
                ...
                .unwrap_or_else(|| {
                    invalid_batch_import_item(
                        item_id.clone(),
                        source.clone(),
                        label.clone(),
                        "未找到有效的 Codex Token".to_string(),
                    )
                    .preview
                })
        }
        CodexBatchImportCandidate::TokenText(raw) => {
            let value = serde_json::Value::String(raw.clone());
            extract_codex_import_candidate_from_value(&value)
                .map(|candidate| {
                    preview_import_candidate(
                        item_id.clone(),
                        source.clone(),
    ...
    CodexBatchImportWorkItem {
        candidate: preview.selectable.then_some(candidate),
        preview,
        auth_file_plan_type,
    }
}

fn collect_batch_import_file_items(
    session_id: &str,
    file_path: &str,
    ordinal: &mut usize,
) -> Vec<CodexBatchImportWorkItem> {
    let path = Path::new(file_path);
    let source = batch_import_source_label(path, file_path);
    let file_label = batch_import_file_stem_label(path);
    let auth_file_plan_type = detect_auth_file_plan_type_from_path(path);
    let content = match fs::read_to_string(path) {
        Ok(content) => content,
        Err(error) => {
            *ordinal += 1;
            return vec![invalid_batch_import_item(
                candidate_item_id(session_id, *ordinal),
                source,
                file_label,
                format!("读取文件失败: {}", error),
            )];
    ...
    match serde_json::from_str::<serde_json::Value>(&content) {
        Ok(serde_json::Value::Array(values)) => {
            for (index, value) in values.into_iter().enumerate() {
                *ordinal += 1;
                let label =
                    label_from_json_value(&value, &format!("{} #{}", file_label, index + 1));
                items.push(build_batch_import_work_item(
                    candidate_item_id(session_id, *ordinal),
                    source.clone(),
                    label,
                    batch_candidate_from_json_value(value),
        ...
        Ok(value @ serde_json::Value::Object(_)) => {
            if looks_like_sub2api_export(&value) {
                if let Some(accounts) = value.get("accounts").and_then(|item| item.as_array()) {
                    for (index, account) in accounts.iter().enumerate() {
                        if !is_sub2api_codex_oauth_account(account) {
                            continue;
                        }
                        *ordinal += 1;
                        let label = label_from_json_value(
                            account,
                            &format!("{} #{}", file_label, index + 1),
                        );
                        items.push(build_batch_import_work_item(
                            candidate_item_id(session_id, *ordinal),
                            source.clone(),
                            label,
                            batch_candidate_from_json_value(account.clone()),
                            auth_file_plan_type.clone(),
                        ));
                    }
                }
            ...
            if items.is_empty() {
                *ordinal += 1;
                let label = label_from_json_value(&value, &file_label);
                items.push(build_batch_import_work_item(
                    candidate_item_id(session_id, *ordinal),
                    source,
                    label,
        ...
        Ok(_) => {
            *ordinal += 1;
            items.push(invalid_batch_import_item(
                candidate_item_id(session_id, *ordinal),
                source,
                file_label,
                "不支持的 JSON 格式".to_string(),
            ));
        }
        Err(_) => {
                ...
                .enumerate()
            {
                *ordinal += 1;
                items.push(build_batch_import_work_item(
                    ...
                    source.clone(),
                    format!("{} #{}", file_label, index + 1),
                    CodexBatchImportCandidate::TokenText(line),
                    auth_file_plan_type.clone(),
                ));
            }
            if items.is_empty() {
...
fn is_batch_import_cancelled(session_id: &str) -> bool {
...
fn scan_codex_batch_import_session(session_id: String) {
    let file_paths = match CODEX_BATCH_IMPORT_SESSIONS.lock() {
        Ok(sessions) => sessions
            .get(&session_id)
            .map(|session| session.file_paths.clone())
            .unwrap_or_default(),
        Err(_) => Vec::new(),
    };
    ...
    publish_batch_import_progress(&session_id, "scanning", 0, total, &all_items, None);

    for (index, file_path) in file_paths.iter().enumerate() {
        if is_batch_import_cancelled(&session_id) {
            break;
        }
        let items = collect_batch_import_file_items(&session_id, file_path, &mut ordinal);
        let current_label = items
            .last()
            .map(|item| item.preview.label.clone())
            .or_else(|| Some(file_path.clone()));
        all_items.extend(items);
        if let Ok(mut sessions) = CODEX_BATCH_IMPORT_SESSIONS.lock() {
            if let Some(session) = sessions.get_mut(&session_id) {
                session.items = all_items.clone();
            }
        }
        let preview = build_batch_import_preview(&session_id, "scanning", &all_items);
        publish_codex_batch_import_event("codex:batch-import-preview", &preview);
        publish_batch_import_progress(
            &session_id,
            "scanning",
            index + 1,
            total,
    ...
    let status = if is_batch_import_cancelled(&session_id) {
        "cancelled"
    } else {
        "ready"
    };
    let preview = match CODEX_BATCH_IMPORT_SESSIONS.lock() {
        Ok(mut sessions) => {
            if let Some(session) = sessions.get_mut(&session_id) {
                session.status = status.to_string();
                session.items = all_items;
                build_batch_import_preview(&session_id, status, &session.items)
            } else {
                build_batch_import_preview(&session_id, "cancelled", &[])
            }
        }
        Err(_) => build_batch_import_preview(&session_id, "cancelled", &[]),
    };
    publish_batch_import_progress(
        &session_id,
        status,
        ...
        &preview
            ...
            .iter()
            .cloned()
            .map(|preview| CodexBatchImportWorkItem {
                preview,
                candidate: None,
                auth_file_plan_type: None,
            })
    ...
    publish_codex_batch_import_event("codex:batch-import-completed", &preview);
}
...
pub fn start_codex_batch_import_from_files(
    ...
    let session_id = format!("codex_batch_{}", uuid::Uuid::new_v4().simple());
    ...
    CODEX_BATCH_IMPORT_SESSIONS
        .lock()
        .map_err(|_| "批量导入会话锁定失败".to_string())?
        .insert(session_id.clone(), session);

    let worker_session_id = session_id.clone();
    std::thread::spawn(move || scan_codex_batch_import_session(worker_session_id));
...
pub fn cancel_codex_batch_import(session_id: &str) -> Result<(), String> {
    let preview = {
        let mut sessions = CODEX_BATCH_IMPORT_SESSIONS
            .lock()
            .map_err(|_| "批量导入会话锁定失败".to_string())?;
        let session = sessions
            .get_mut(session_id)
            .ok_or_else(|| format!("批量导入会话不存在: {}", session_id))?;
        session.cancel_requested = true;
        if session.status != "scanning" {
            session.status = "cancelled".to_string();
            Some(build_batch_import_preview(
                session_id,
                &session.status,
                &session.items,
            ))
...
pub fn resume_codex_batch_import(session_id: &str) -> Result<(), String> {
    let action = {
        let mut sessions = CODEX_BATCH_IMPORT_SESSIONS
            .lock()
            .map_err(|_| "批量导入会话锁定失败".to_string())?;
        ...
        match session.status.as_str() {
            "scanning" => {
                let preview =
                    build_batch_import_preview(session_id, &session.status, &session.items);
                EitherBatchImportResumeAction::Publish(preview)
            }
            "ready" => {
                let preview =
                    build_batch_import_preview(session_id, &session.status, &session.items);
            ...
            _ => {
                ...
                session.items.clear();
                EitherBatchImportResumeAction::Restart
            }
        }
    };
...
enum EitherBatchImportResumeAction {
    Publish(CodexBatchImportPreview),
    Restart,
}

pub fn get_codex_batch_import_preview(session_id: &str) -> Result<CodexBatchImportPreview, String> {
    let sessions = CODEX_BATCH_IMPORT_SESSIONS
        .lock()
        .map_err(|_| "批量导入会话锁定失败".to_string())?;
    let session = sessions
        .get(session_id)
        .ok_or_else(|| format!("批量导入会话不存在: {}", session_id))?;
    Ok(build_batch_import_preview(
        session_id,
        &session.status,
        &session.items,
    ))
...
fn block_on_batch_import_future<T, F>(future: F) -> Result<T, String>
where
    T: Send + 'static,
    F: Future<Output = Result<T, String>> + Send + 'static,
{
    ...
    .join()
    .map_err(|_| "批量导入异步任务崩溃".to_string())?
}

fn import_batch_candidate(
    candidate: CodexBatchImportCandidate,
    ...
    match candidate {
        CodexBatchImportCandidate::LegacyTokens {
            tokens,
            account_id_hint,
        } => upsert_account_with_hints(tokens, account_id_hint, None).map(|account| vec![account]),
        CodexBatchImportCandidate::JsonValue(value) => block_on_batch_import_future(async move {
            import_account_from_json_value(value)
                .await
                .map(|account| account.into_iter().collect())
        }),
        CodexBatchImportCandidate::TokenText(raw) => block_on_batch_import_future(async move {
            let value = serde_json::Value::String(raw);
            let candidate = extract_codex_import_candidate_from_value(&value)
                .ok_or_else(|| "未找到有效的 Codex Token".to_string())?;
            import_codex_candidate(candidate)
                .await
...
pub fn confirm_codex_batch_import(
    ...
    ensure_storage_writable_for_import()?;
    let selected: HashSet<&str> = item_ids.iter().map(|item| item.as_str()).collect();
    if selected.is_empty() {
        return Err("未选择任何账号".to_string());
    }
    let items = {
        let sessions = CODEX_BATCH_IMPORT_SESSIONS
            .lock()
        ...
        let session = sessions
            ...
            .ok_or_else(|| format!("批量导入会话不存在: {}", session_id))?;
        if session.status == "scanning" {
            return Err("批量导入仍在扫描中".to_string());
        }
        session.items.clone()
    };
    ...
    for item in items
        .into_iter()
        .filter(|item| selected.contains(item.preview.item_id.as_str()))
    {
        let label = item
            .preview
            .email
            .clone()
            .unwrap_or_else(|| item.preview.label.clone());
        let Some(candidate) = item.candidate else {
            failed.push(CodexFileImportFailure {
                ...
                error: item
                    ...
                    .error
                    .clone()
                    .unwrap_or_else(|| "账号不可导入".to_string()),
            });
            continue;
        };
        match import_batch_candidate(candidate) {
            Ok(accounts) => {
                for mut account in accounts {
                    if apply_auth_file_plan_type(&mut account, item.auth_file_plan_type.clone()) {
                        save_account(&account)?;
                    }
            ...
            Err(error) => {
                if is_disk_full_error_message(&error) {
                    return Err(format!(
                        "磁盘空间不足，已终止导入（已成功 {} 个）。{}",
                        imported.len(),
                        error
...
pub async fn import_from_files(file_paths: Vec<String>) -> Result<CodexFileImportResult, String> {
    ...
    for (tokens, account_id_hint, label, auth_file_plan_type) in candidates {
        progress_index += 1;
        publish_file_import_progress(progress_index, total, &label);
