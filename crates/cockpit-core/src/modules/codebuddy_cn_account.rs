...
fn ensure_instance_state_db_path(user_data_dir: &str) -> Result<PathBuf, String> {
    ...
    if let Some(parent) = preferred.parent() {
        fs::create_dir_all(parent)
            .map_err(|err| format!("create CodeBuddy CN globalStorage failed: {err}"))?;
    }
    if let Some(default_db) = get_default_codebuddy_cn_state_db_path() {
...
fn verify_instance_state_db_injection(state_db_path: &Path, db_key: &str) -> Result<(), String> {
    let conn = rusqlite::Connection::open(state_db_path)
        .map_err(|err| format!("open CodeBuddy CN state.vscdb failed: {err}"))?;
    ...
    match value {
        ...
        _ => Err(format!(
            "CodeBuddy CN account injection verification failed: db={}, key={}",
...
pub fn inject_to_codebuddy_cn_user_data_dir(
    user_data_dir: &str,
    account_id: &str,
) -> Result<(), String> {
    let account =
